...
//                UPS_Ctr_Info.PWM_Period = User_Divider(64000000, (INV_Ctrl_Info.PWM_Freq_Init+(UPS_Ctr_Info.delta_T))*2);//计数方式：三角波
    ...
    //限制工作周期
    if(INV_Ctrl_Info.PWM_Period < INV_Ctrl_Info.PWM_Period_67Hz) INV_Ctrl_Info.PWM_Period = INV_Ctrl_Info.PWM_Period_67Hz;
    if(INV_Ctrl_Info.PWM_Period > INV_Ctrl_Info.PWM_Period_43Hz) INV_Ctrl_Info.PWM_Period = INV_Ctrl_Info.PWM_Period_43Hz;              
}
...
//            INV_Repeat_Ctrl(&INV_Repeat_Info);
            ...
            //误差值限制
            if(INV_Repeat_Info.err > INV_Repeat_Info.err_Max)    INV_Repeat_Info.err =  INV_Repeat_Info.err_Max;
            if(INV_Repeat_Info.err < INV_Repeat_Info.err_Min)    INV_Repeat_Info.err =  INV_Repeat_Info.err_Min;
            ...
            if(INV_Ctrl_Info.periodDot_Cnt < (INV_Repeat_Info.periodDot_Val - INV_Repeat_Info.lag_Point))
            {
                INV_Repeat_Info.out = (INV_Repeat_Info.Qr_Coeff * RepeatOut[INV_Ctrl_Info.periodDot_Cnt] + \
                                    INV_Repeat_Info.Kr_Coeff * RepeatErr[INV_Repeat_Info.lag_Point + INV_Ctrl_Info.periodDot_Cnt]) >> 12;
            }
            else
            {
                INV_Repeat_Info.out = (INV_Repeat_Info.Qr_Coeff * RepeatOut[INV_Ctrl_Info.periodDot_Cnt] + \
                                    INV_Repeat_Info.Kr_Coeff * RepeatErr[INV_Ctrl_Info.periodDot_Cnt - \
                                    INV_Repeat_Info.periodDot_Val + INV_Repeat_Info.lag_Point]) >> 12;
            }
            ...
            //输出值限制
            if(INV_Repeat_Info.out > INV_Repeat_Info.out_Max)    INV_Repeat_Info.out =  INV_Repeat_Info.out_Max;
            if(INV_Repeat_Info.out < INV_Repeat_Info.out_Min)    INV_Repeat_Info.out =  INV_Repeat_Info.out_Min;             
...
//        if ( INV_PID_Vol.err < INV_PID_Vol.err_Min ) INV_PID_Vol.err = INV_PID_Vol.err_Min;
        ...
        /*----------------------------------PID输出限幅--------------------------------------------*/
        if ( INV_PID_Vol.out > INV_PID_Vol.out_Max ) INV_PID_Vol.out = INV_PID_Vol.out_Max;
        if ( INV_PID_Vol.out < INV_PID_Vol.out_Min ) INV_PID_Vol.out = INV_PID_Vol.out_Min;
...
//        if ( INV_PID_Cur.err < INV_PID_Cur.err_Min ) INV_PID_Cur.err = INV_PID_Cur.err_Min;
            ...
            /*----------------------------------PID输出限幅--------------------------------------------*/
            if ( INV_PID_Cur.out > INV_PID_Cur.out_Max ) INV_PID_Cur.out = INV_PID_Cur.out_Max;
            if ( INV_PID_Cur.out < INV_PID_Cur.out_Min ) INV_PID_Cur.out = INV_PID_Cur.out_Min;
    ...
    {    
        ...
        /*----------------------------------PID输出限幅--------------------------------------------*/
        if ( INV_PID_DCIM.out > INV_PID_DCIM.out_Max ) INV_PID_DCIM.out = INV_PID_DCIM.out_Max;
        if ( INV_PID_DCIM.out < INV_PID_DCIM.out_Min ) INV_PID_DCIM.out = INV_PID_DCIM.out_Min;
