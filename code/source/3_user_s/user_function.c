...
//                    State_Context.flag.bit.initial_Ok       = E_FALSE;            
    ...
    {        
        if(SysClockBase_ms.sys_1ms == 1)    
        {
            if(PFC_Ctrl_Info.PFC_FreqOK_Cnt <= 1000)  PFC_Ctrl_Info.PFC_FreqOK_Cnt++;                
        }            
...
{  
    ...
    if(COM_AD_Data_Info.VACIN_Freq_Val > 700) COM_AD_Data_Info.VACIN_Freq_Val = 700;
    if(COM_AD_Data_Info.VACOUT_Freq_Val > 700) COM_AD_Data_Info.VACOUT_Freq_Val = 700;
